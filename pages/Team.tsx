...
export const Team: React.FC = () => {
  ...
  const [role, setRole] = useState<UserRole>(UserRole.SUB_CUSTODY);
  ...
  const userDetails = useMemo(() => {
      ...
      if (currentUser.role === UserRole.ADMIN && selectedUser.role === UserRole.MAIN_CUSTODY) {
  ...
  const renderTeamView = () => {
    if (currentUser.role === UserRole.ADMIN) {
        const engineers = users.filter(u => u.role === UserRole.MAIN_CUSTODY);
        return (
            <div className="grid grid-cols-1 gap-8">
                {engineers.map(engineer => {
                    const technicians = users.filter(u => u.role === UserRole.SUB_CUSTODY && u.managerId === engineer.id);
                    return (
                        <div key={engineer.id} className="bg-white dark:bg-slate-800 rounded-3xl shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700 overflow-hidden group hover:shadow-xl transition-shadow duration-300">
                             <div className="p-8 flex flex-col md:flex-row items-start md:items-center justify-between bg-gradient-to-r from-slate-50 to-white dark:from-slate-800 dark:to-slate-800/50 border-b border-slate-100 dark:border-slate-700 gap-4">
                                 <div className="flex items-center gap-5 cursor-pointer" onClick={() => setSelectedUser(engineer)}>
                                     ...
                                     <div><h3 className="font-black text-xl text-slate-800 dark:text-white flex items-center gap-3">{engineer.name}<span className="text-[10px] bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 px-2.5 py-1 rounded-full border border-blue-200 dark:border-blue-800 uppercase tracking-wider">{engineer.jobTitle || t('roleMainCustody')}</span></h3><p className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1">{engineer.email}</p></div>
                                 </div>
                                 <div className="flex items-center gap-3 self-end md:self-auto"><div className="bg-white dark:bg-slate-700 px-4 py-1.5 rounded-full text-xs font-bold text-slate-500 dark:text-slate-300 border border-slate-200 dark:border-slate-600 shadow-sm">{technicians.length} {t('techniciansCount')}</div><button onClick={(e) => { e.stopPropagation(); deleteUser(engineer.id); }} className="text-slate-300 hover:text-red-500 p-2 transition-colors hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full" title={t('deleteEngineer')}><Trash2 size={20} /></button></div>
                             </div>
                             <div className="p-8 bg-white dark:bg-slate-800"><h4 className="text-xs font-bold text-slate-400 dark:text-slate-500 mb-5 flex items-center gap-2 uppercase tracking-widest"><ArrowRight size={14} /> {t('subTeam')}</h4>
                                 {technicians.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{technicians.map(tech => (<div key={tech.id} onClick={() => setSelectedUser(tech)} className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 hover:border-blue-300 dark:hover:border-blue-500 hover:shadow-md cursor-pointer transition-all duration-200 group/tech"><img src={tech.avatarUrl || getStableAvatar(tech.name)} className="w-12 h-12 rounded-full shadow-sm object-cover" alt={tech.name} /><div className="flex-1 overflow-hidden"><p className="font-bold text-sm text-slate-800 dark:text-white truncate group-hover/tech:text-blue-600 transition-colors">{tech.name}</p><p className="text-xs text-slate-500 dark:text-slate-400 truncate font-medium">{tech.jobTitle || t('roleSubCustody')}</p></div><button onClick={(e) => { e.stopPropagation(); deleteUser(tech.id); }} className="text-slate-300 hover:text-red-500 opacity-0 group-hover/tech:opacity-100 transition-opacity"><Trash2 size={16} /></button></div>))}</div>) : (<div className="text-center py-8 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700"><p className="text-xs text-slate-400 italic">{t('noTechnicians')}</p></div>)}
                             </div>
                        </div>
                    );
                })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {users.map(u => (
                <div key={u.id} onClick={() => setSelectedUser(u)} className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm hover:shadow-xl border border-slate-100 dark:border-slate-700 flex flex-col gap-5 relative group cursor-pointer hover:-translate-y-1 transition-all duration-300">
                    ...
                    <div className="pt-4 border-t border-slate-50 dark:border-slate-700 flex justify-between items-center"><span className="text-xs font-bold px-3 py-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-full">{u.jobTitle || (u.role === UserRole.MAIN_CUSTODY ? t('roleMainCustody') : t('roleSubCustody'))}</span></div>
                </div>
            ))}
        </div>
    );
    ...
    <div className="space-y-8 animate-fade-in pb-20">
           ...
           <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-md animate-scale-in flex flex-col max-h-[90vh] border border-white/10 modal-overlay">
             ...
             <div className="p-8 overflow-y-auto custom-scrollbar flex-1">
                 <form onSubmit={handleAdd} className="space-y-5">
                     ...
                     <div><label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wide">{t('jobTitle')}</label><input type="text" value={jobTitle} onChange={e => setJobTitle(e.target.value)} className="input-modern w-full p-3.5 bg-slate-50 dark:bg-slate-800 rounded-xl outline-none text-slate-900 dark:text-white font-medium" /></div>
                     <div>
                         <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wide">{t('role')}</label>
                         <div className="relative">
                             <select value={role} onChange={e => setRole(e.target.value as UserRole)} className="input-modern w-full p-3.5 bg-slate-50 dark:bg-slate-800 rounded-xl outline-none text-slate-900 dark:text-white font-medium appearance-none">
                                 {currentUser.role === UserRole.ADMIN && <option value={UserRole.MAIN_CUSTODY}>{t('engineerProjectSupervisor')}</option>}
                                 <option value={UserRole.SUB_CUSTODY}>{t('technicianAssistant')}</option>
                             </select>
      ...
      {selectedUser && userDetails && (
         <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/70 backdrop-blur-md p-4 animate-fade-in">
             <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-4xl animate-scale-in flex flex-col max-h-[90vh] border border-white/10 modal-overlay">
                {/* Header */}
                <div className="flex justify-between items-start p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                     <div className="flex gap-6">
                        ...
                        <div>
                            ...
                            <p className="text-blue-600 dark:text-blue-400 font-bold mb-2">{selectedUser.jobTitle || (selectedUser.role === UserRole.MAIN_CUSTODY ? t('roleMainCustody') : t('roleSubCustody'))}</p>
                            {selectedUser.phone && <p className="text-sm text-slate-500 flex items-center gap-2 mb-1"><Phone size={14}/> {selectedUser.phone}</p>}
                            <p className="text-sm text-slate-500 flex items-center gap-2"><Briefcase size={14}/> {userDetails.hierarchy.length} {t('activeProjectsCount')}</p>
                        </div>
                     </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                    ...
                    {userDetails.teamMembers.length > 0 && (
                        <div className="mb-8">
                            <h4 className="text-lg font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2"><Users size={20}/> {t('team')}</h4>
                                ...
                                {userDetails.teamMembers.map(tm => (
                                    <div key={tm.id} className="bg-slate-50 dark:bg-slate-800 p-3 rounded-xl flex items-center gap-3 border border-slate-100 dark:border-slate-700">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500">{tm.jobTitle || t('roleSubCustody')}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <h4 className="text-lg font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2"><Wallet size={20}/> {t('financialRecords')}</h4>
                    
                    {userDetails.hierarchy.length === 0 ? (<div className="text-center py-12 text-slate-400 dark:text-slate-500 flex flex-col items-center bg-slate-50 dark:bg-slate-800/50 rounded-2xl"><FileText size={48} className="mb-4 opacity-20"/><p className="font-medium italic">{t('noFinancialRecords')}</p></div>) : (
                        userDetails.hierarchy.map((item) => (
                            <div key={item.project?.id} className="border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm">
                                <div onClick={() => setExpandedProject(expandedProject === item.project?.id ? null : item.project?.id || '')} className="bg-slate-50 dark:bg-slate-800 p-5 flex justify-between items-center cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                                    <h4 className="font-bold text-lg text-slate-800 dark:text-white flex items-center gap-3"><div className="p-2 bg-blue-100 dark:bg-blue-900/30 rounded-lg text-blue-600"><Briefcase size={18} /></div>{item.project?.name || t('generalAdvanceNoProject')}</h4>
                                    <div className="flex items-center gap-4"><span className="text-sm font-bold text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900 px-3 py-1 rounded-lg border border-slate-200 dark:border-slate-700">{item.advances.length} {t('advances')}</span>{expandedProject === item.project?.id ? <ChevronUp size={20} className="text-slate-400"/> : <ChevronDown size={20} className="text-slate-400"/>}</div>
                                </div>
                                        ...
                                        <div key={adv.id} className="border border-slate-100 dark:border-slate-700 rounded-xl p-4 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                                            <div className="flex justify-between items-center mb-3">
                                                ...
                                                <div className="text-end">
                                                    <p className="font-black text-slate-800 dark:text-white">{adv.amount.toLocaleString()} <span className="text-[10px] text-slate-400">{t('currency')}</span></p>
                                                </div>
                                            </div>
                                            <div className="flex gap-2 text-xs">
                                                <div className="bg-amber-50 text-amber-700 px-2 py-1 rounded flex-1 text-center">{t('projectTotalSpent')}: {adv.spent.toLocaleString()}</div>
                                                <div className="bg-emerald-50 text-emerald-700 px-2 py-1 rounded flex-1 text-center">{t('remaining')}: {adv.remainingAmount.toLocaleString()}</div>
                                            </div>
                                        </div>
                                    ))}
                                </div>)}
